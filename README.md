...
### 5️⃣ 🌐 개별 노드 글로벌 프리셋 관리자
개별 노드의 모든 파라미터를 세밀하게 튜닝하고 알약 태그로 선택 적용하는 독립 관리 시스템입니다.

<p align="center">
  <img src="docs/assets/05_global_preset_modal.png" alt="글로벌 프리셋 관리자" width="850">
</p>

- **선택적 파라미터 토글**: `unet_name`, `weight_dtype`, `seed`, `steps` 등 원하는 위젯 값만 클릭하여 On/Off 가능
- **인라인 스텝퍼 에디팅**: 상세 보기에서 수치를 직접 수정하고 저장 가능
- **ComfyUI 전역 공유**: 동일 노드라면 어떤 워크플로우를 열어도 즉시 동일 프리셋 목록 제공
