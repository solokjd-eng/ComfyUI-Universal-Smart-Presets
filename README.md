...
복잡한 워크플로우의 다중 노드 설정을 통째로 스냅샷 저장하고 캔버스에서 다이렉트로 즉시 전환하는 **🌟 Universal Preset Hub (유니버셜 프리셋 허브)**와 개별 노드별 세밀한 **🌐 Global Smart Presets**를 제공합니다.

---

## 📑 목차 (Table of Contents)
1. [🌟 Universal Preset Hub (유니버셜 프리셋 허브)](#1--universal-preset-hub-유니버셜-프리셋-허브)
...
> **"체크포인트, VAE, CLIP, 다중 LoRA, KSampler, 바이패스(Bypass) 상태까지 — 단 한 번의 클릭으로 전체 워크플로우 셋업을 캔버스에서 즉시 전환하세요!"**

워크플로우가 복잡해질수록 모델 교체나 파라미터 변경 시 여러 노드를 일일이 찾아가며 값을 바꿔야 하는 번거로움이 있습니다.  
`Universal Preset Hub`는 캔버스에서 원하는 여러 노드를 선택하여 **하나의 통합 유니버셜 프리셋**으로 저장하고, 설정창 진입 없이 **캔버스 노드 본체의 라디오 토글 스위치**로 언제든지 단 1초 만에 전체 노드에 설정을 일괄 적용합니다.

---

### 💡 핵심 기능 및 특징

- **🔘 캔버스 원클릭 라디오 스위처 (Fast Groups 스타일)**: 설정창을 열지 않고도 노드 본체에서 직접 프리셋을 클릭하여 ON/OFF 즉시 전환 (단일 활성화 독점 모드)
- **📏 가변 높이 자동 조절 & 캔버스 휠 스크롤**: 1~5개는 알맞게 줄어들고, 6개 이상 시 노드 내 마우스 휠 스크롤로 부드럽게 탐색
- **📡 다중 노드 실시간 무선 미러링**: 한 워크플로우에 허브 노드를 여러 개 배치해 두면 1개처럼 완벽 동기화 (화면 이동 없이 어디서든 제어)
- **🟣 바이패스(Bypass) & 뮤트(Mute) 상태 일괄 저장**: 업스케일러, 디테일러 등의 온/오프 분기를 원클릭으로 일괄 전환
- **다중 노드 일괄 스냅샷**: 캔버스에서 드래그 또는 `Ctrl + 클릭`으로 선택된 모든 노드의 위젯 상태를 완벽 캡처
- **동적 LoRA 스택 & Fast Groups 무손실 저장**: `Power Lora Loader (rgthree)`, `Fast Groups Bypasser (rgthree)` 등 특수 노드 완벽 연동
- **타 워크플로우 100% 호환**: 노드 ID가 달라져도 자동 인식하는 **3단계 스마트 매칭 알고리즘** 탑재
- **스마트 자동 동봉**: 워크플로우 저장(`Ctrl+S`) 및 생성 이미지(PNG) 메타데이터에 프리셋 정보 자동 동봉 (다른 워크플로우 유출 0%)
...
- **ComfyUI 시스템 전역(Global) 영구 저장**: 개별 워크플로우 파일이 아닌 ComfyUI 시스템에 전역 저장되므로, **동일한 종류의 노드(예: KSampler, Checkpoint Loader 등)라면 다른 어떤 워크플로우를 열어도 별도의 내보내기/가져오기 없이 프리셋이 즉시 공유·적용**됩니다.
...
- **내장 가이드 모달**: 허브 관리자 하단의 **`💡 다른 워크플로우 적용/백업 가이드`** 버튼을 통해 타 워크플로우 매칭 팁과 다중 동일 노드 제목 지정 꿀팁을 언제든 확인 가능
